...
def load_data():
    ...
    merged = frames["btc_usd_cb"]
    for key in ["btc_usdt_bn", "btc_usdc_bn", "btc_usdc_cb", "btc_usdt_cb", "usdt_usd_cb"]:
        if key in frames:
            merged = merged.merge(frames[key], on="timestamp", how="inner")

    merged = merged.sort_values("timestamp").reset_index(drop=True)

    merged["implied_usdc_cb"] = merged["btc_usd_cb_close"] / merged["btc_usdc_cb_close"]
    merged["implied_usdc_bn"] = merged["btc_usd_cb_close"] / merged["btc_usdc_bn_close"]
    merged["usdc_discount_cb_bps"] = (merged["implied_usdc_cb"] - 1.0) * 10000
    merged["usdc_discount_bn_bps"] = (merged["implied_usdc_bn"] - 1.0) * 10000
...
def _chart(fig, title, yaxis_title, height=420):
    fig.update_layout(
...
def render():
    ...
    **Index:**
    ...
    st.header("Act I: The Fracture (March 10–11, 2023)")
    ...
    fig1.update_yaxes(title_text="Coinbase Volume (BTC)", secondary_y=False)
    fig1.update_yaxes(title_text="Binance Volume (BTC)", secondary_y=True, showgrid=False)
    _chart(fig1, "Volume Fracture: Coinbase vs Binance (BTC/USDC)", "", height=450)
    st.plotly_chart(fig1, use_container_width=True)

    crisis_df = df[(df["timestamp"] >= svb_start) & (df["timestamp"] <= svb_end)]
    pre_crisis = df[df["timestamp"] < svb_start]

    bn_vol_crisis = crisis_df["btc_usdc_bn_vol"].sum()
    bn_vol_pre = pre_crisis["btc_usdc_bn_vol"].sum() / max(1, len(pre_crisis)) * len(crisis_df)
    cb_vol_crisis = crisis_df["btc_usdc_cb_vol"].sum()
    cb_vol_pre = pre_crisis["btc_usdc_cb_vol"].sum() / max(1, len(pre_crisis)) * len(crisis_df)
    ...
    st.header("Act II: The Two Exit Doors")
    ...
    fig2.add_trace(go.Scatter(
        x=df["timestamp"], y=df["usdc_discount_cb_bps"],
        name="USDC Discount (Coinbase)", line=dict(color="#FF6B6B", width=1.5),
    ))
    fig2.add_trace(go.Scatter(
        x=df["timestamp"], y=df["usdc_discount_bn_bps"],
        name="USDC Discount (Binance)", line=dict(color="#FFD93D", width=1.5),
    ))
    fig2.add_hline(y=0, line_dash="dash", line_color="white", annotation_text="Parity")
    fig2.add_vrect(
        x0=svb_start, x1=svb_end,
        fillcolor="rgba(255,107,107,0.15)", line_width=0,
        annotation_text="SVB Weekend", annotation_position="top left",
    )
    _chart(fig2, "USDC Discount: Coinbase vs Binance (bps)", "Basis Points")
    st.plotly_chart(fig2, use_container_width=True)

    peak_discount_cb = crisis_df["usdc_discount_cb_bps"].min()
    peak_discount_bn = crisis_df["usdc_discount_bn_bps"].min()
    ...
    st.header("Act III: The Aftermath — Fee Structures & Custodial Shifts")
    ...
    st.header("Act IV: The GENIUS Act — Does It Solve the Problem?")
    ...
    fig3.add_hline(y=0, line_dash="dash", line_color="white", annotation_text="Parity")
    fig3.add_vrect(
        x0=svb_start, x1=svb_end,
        fillcolor="rgba(255,107,107,0.15)", line_width=0,
        annotation_text="SVB Weekend", annotation_position="top left",
    )
    _chart(fig3, "The Weekend Gap: Onshore/Offshore Spread by Day Type", "Spread (bps)")
